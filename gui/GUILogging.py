...
class GUILogging(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args):
        ...
        fmt = logging.Formatter('%(asctime)s - %(levelname)-10s %(message)s', "%H:%M:%S")

        stream_handler = logging.StreamHandler(stream=self)
        stream_handler.setFormatter(fmt)
        stream_handler.setLevel(logging.INFO)

        logging.getLogger('pyfitstat').addHandler(stream_handler)
        logging.getLogger('garminexport').addHandler(stream_handler)
    ...
    def create_toolbar(self):
        ...
        self.clear_logging_action = QtWidgets.QAction(
            QtGui.QIcon(os.path.join(os.getcwd(), 'icons', 'clear.png')), 'Clear', self)
